...
const UserMessages = () => {
  ...
  // Get user ID based on the logged-in user
  ...
  // Fetch users by role (users with the role 'user') and vendors
  const fetchUsersAndVendors = async () => {
    setLoading(true);
    setError(null);
    try {
      const [usersResponse, vendorsResponse] = await Promise.all([
        axios.get(`${BACKEND_URL}/api/users/role/user`, { headers: { Authorization: authHeader } }),
        axios.get(`${BACKEND_URL}/api/users/role/vendor`, { headers: { Authorization: authHeader } }),
      ]);

      if (usersResponse.data && vendorsResponse.data) {
        setUsers([...usersResponse.data, ...vendorsResponse.data]); // Combine users and vendors
      } else {
        setError('No users or vendors found');
      }
    } catch (err) {
      setError('Failed to fetch users and vendors.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchMessages = async () => {
    ...
    try {
      const response = await axios.post(
          `${BACKEND_URL}/api/threads/messages/create/${userId}/${receiverId}`,
  ...
  const renderUserCard = ({ item }) => (
      <View style={styles.card}>
        <Text style={styles.title}>
          {item.role === 'vendor' ? 'Vendor' : 'User'}: {item.firstName} {item.lastName}
        </Text>
        <TouchableOpacity
  ...
  return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.container}>
          ...
          <FlatList
              data={users}
              keyExtractor={(item) => `${item.userId}-${item.firstName}-${item.lastName}`}
              renderItem={renderUserCard}
              ListEmptyComponent={<Text style={styles.emptyText}>No users or vendors found.</Text>}
          />
          {/* Message Modal */}
              ...
              <FlatList
                  ...
                  renderItem={({ item }) => (
                      <View style={[styles.messageContainer, item.senderId === userId ? styles.sentMessage : styles.receivedMessage]}>
                        <Text style={[styles.messageText, item.senderId === userId ? styles.sentText : styles.receivedText]}>
                          {item.content}
                        </Text>
                      </View>
                  )}
